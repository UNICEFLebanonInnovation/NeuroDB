...
class SocialAccountAdapter(DefaultSocialAccountAdapter):
    ...
    def pre_social_login(self, request, sociallogin):
        ...
        if sociallogin.is_existing:
            return
        email = (sociallogin.account.extra_data.get("mail") or sociallogin.account.extra_data.get("userPrincipalName") or "")
        email = email.strip().lower()
        user = User.objects.filter(email__iexact=email, is_active=True).first() if email else None
        if user is None:
            messages.error(request, "Your Microsoft account is not registered for NeuroDB. Ask an administrator for access.")
