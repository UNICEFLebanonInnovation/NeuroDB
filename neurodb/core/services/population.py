...
def _matrix(qs, row_field: str, col_field: str) -> dict[str, Any]:
    ...
    return {
        "columns": col_list,
        "rows": [{"label": k, "values": [v.get(c, 0) for c in col_list], "total": sum(v.values())} for k, v in sorted(rows.items())],
...
def population_view(year: int, category: str = "total") -> dict[str, Any]:
    ...
    return {
        "year": year,
        "category": category,
        "totals_by_nationality": {r["nationality"]: r["value"] for r in national.values("nationality", "value")},
        "by_governorate": _matrix(base.filter(level="governorate", age_group="", sex="", vulnerability_level=""), "area_name", "nationality"),
        "by_district": _matrix(base.filter(level="district", age_group="", sex="", vulnerability_level=""), "area_name", "nationality"),
        "by_age_group": _matrix(base.filter(level="national", sex="").exclude(age_group=""), "age_group", "nationality"),
        "by_vulnerability": _matrix(base.filter(level="district").exclude(vulnerability_level=""), "area_name", "vulnerability_level")
